...
bintree<int> resolver(bintree<int> const& arbol1, bintree<int> arbol2) {
	int raiz = -0;
	if (arbol1.empty() && arbol2.empty()) {
		return{arbol1, -1, arbol2};
	}
	else {
		bintree<int> izq, der;
		...
		if (arbol1.empty()) {
			izq = resolver(arbol1, arbol2.left());
			der = resolver(arbol1, arbol2.right());
		}
		else if (arbol2.empty()) {
			izq = resolver(arbol1.left(), arbol2);
			der = resolver(arbol1.right(), arbol2);
		}
		else {
			izq = resolver(arbol1.left(), arbol2.left());
			der = resolver(arbol1.right(), arbol2.right());
		}
		return{ izq, raiz, der };
	}
}
bool resuelveCaso() {
	...
	bintree<int> arbol1 = cargarArbol(-1);
	bintree<int> arbol2 = cargarArbol(-1);
	if (arbol1.empty() && arbol2.empty())
		return false;
	bintree<int> solucion = resolver(arbol1, arbol2);
	std::vector<int> sol = solucion.preorder();
