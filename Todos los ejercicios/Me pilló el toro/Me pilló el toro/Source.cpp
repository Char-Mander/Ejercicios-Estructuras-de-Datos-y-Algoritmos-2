...
public:
	notas() {}
	void anadirNota(const std::string &a, const std::string &nota) {
		//++tabla[a];
		if (tabla.contains(a)) {
			if (nota == "CORRECTO") ++tabla[a];
			else --tabla[a];

		}
		else {
			if(nota == "CORRECTO") tabla.insert({ a, 1 });
	...
	void mostrar() {
		for (auto &it : tabla) {
			if (it.valor != 0) std::cout << it.clave << ", " << it.valor << '\n';
		}
	}
};
